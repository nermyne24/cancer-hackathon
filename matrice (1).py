...
# ✅ AJOUT : import des outils pour la matrice de confusion
...
# ─────────────────────────────────────────────
# 6.5. MATRICE DE CONFUSION (Figure ajoutée)
# ─────────────────────────────────────────────
...
ax.set_title(f'Matrice de Confusion - Test (20%)\nAccuracy : {test_acc:.2f}%',
             fontsize=13, fontweight='bold')
plt.tight_layout()
plt.savefig('confusion_matrix_training.png', dpi=150)
plt.show()
print("✅ Figure sauvegardee : confusion_matrix_training.png")

# ─────────────────────────────────────────────
# 6.6. COURBES ACCURACY ET LOSS PAR EPOQUE (NOUVELLE VERSION CORRIGEE)
# ─────────────────────────────────────────────

print("\nGeneration des courbes d'evolution par epoch...")

# Split train en train/val pour le tracking (meme fraction que le modele)
...
# Creation d'un modele pour tracking epoch par epoch
...
# Entraînement epoch par epoch pour tracker les métriques
n_epochs = len(model.loss_curve_)
print(f"Nombre d'epochs a tracker : {n_epochs}")
...
for epoch in range(n_epochs):
    ...
    y_train_proba = model_tracking.predict_proba(X_train_track)
    train_loss_history.append(log_loss(y_train_track, y_train_proba))
    
    # Loss validation
    y_val_proba = model_tracking.predict_proba(X_val_track)
    val_loss_history.append(log_loss(y_val_track, y_val_proba))
    
    if (epoch + 1) % 20 == 0:
        print(f"  Epoch {epoch+1}/{n_epochs} - Train Acc: {train_acc_history[-1]:.3f} - Val Acc: {val_acc_history[-1]:.3f}")
...
# Creation de la figure avec 2 subplots cote a cote
fig, axes = plt.subplots(1, 2, figsize=(14, 5.5))
fig.suptitle('Classification Cancer - Deep Learning (Version Realiste)', 
             fontsize=14, fontweight='bold')

# Sous-graphique 1: Accuracy par epoque
axes[0].plot(train_acc_history, color='#1f77b4', linewidth=2, label='Train')
axes[0].plot(val_acc_history, color='#ff7f0e', linewidth=2, linestyle='--', label='Val')
axes[0].axhline(y=test_acc_normalized, color='#d62728', linewidth=1.5, 
                linestyle=':', label=f'Test={test_acc:.1f}%')
axes[0].set_xlabel('Epoque', fontsize=11)
axes[0].set_ylabel('Accuracy', fontsize=11)
axes[0].set_title('Accuracy par epoque', fontsize=12)
axes[0].legend(loc='upper left', fontsize=9)
axes[0].grid(True, alpha=0.3)
axes[0].set_ylim([0, 1.0])
axes[0].set_xlim([0, n_epochs])

# Sous-graphique 2: Loss par epoque
axes[1].plot(train_loss_history, color='#1f77b4', linewidth=2, label='Train')
axes[1].plot(val_loss_history, color='#ff7f0e', linewidth=2, linestyle='--', label='Val')
axes[1].set_xlabel('Epoque', fontsize=11)
axes[1].set_ylabel('Loss', fontsize=11)
axes[1].set_title('Loss par epoque', fontsize=12)
...
plt.show()
print("✅ Courbes d'entrainement sauvegardees : training_curves.png")
